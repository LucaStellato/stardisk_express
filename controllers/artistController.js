...
function showArtist(req, res) {
    ...
    const sql =
        `SELECT artists.name AS artist, artists.record_label, artists.description AS artist_description ,artists.img_url AS artist_img, artists.id AS artist_id, products.name AS name, products.full_price, products.img_url
        FROM artists
